...
class ETE_ETV_Net(tf.keras.Model):
    ...
    def ETV(self, inputs, batch_size, trainable_ETE_ETV, reg_lambdas, con_norm, dropout_rate):
        '''
    ...
    def build(self, batch_size, net_type = 1, trainable_ETE_ETV = True, num_ETE = 1, num_neurons = 10, reg_lambdas = (0.0, 0.0), con_norm = 1000., dropout_rate = 0.0):
        ...
        if net_type == 1:
            ...
            x = tf.math.multiply(x, Filters.mark_start_filter(shape=(self.n, self.n, 1))) # MS
            x = tf.math.multiply(x, x) # MSE

            # Mark end and Mark end edge
            out = tf.math.multiply(x, Filters.mark_end_filter(shape=(self.n, self.n, 1))) # ME
            x = tf.math.multiply(x, x) # MEE
            out = x
