...
def main(num, now_testing, comp_list, net_type = 1):
    file_dir = current_file_directory + '/new_magic_results/' + now_testing
    #file_dir = current_file_directory + '/yu_new_magic/'
    names = ['c', 'q', 'positive', 'negative', 'T', 'H']
    
    num_classes = len(comp_list)
    names = [names[x] for x in comp_list]

    data_X, data_labels, data_ranking = load_data( current_file_directory + '/new_magic_results/dataset/' + str(n) + '.npz')    
    #data_X, data_labels, data_ranking = load_data( current_file_directory + '/yu_new_magic/' + '1000random25' + '.npz')    
    ...
    #data_X, data_labels = rinse_out(data_X, data_labels, np.array([1., 0., 0.]), 1000)
    ...
    #q_count_train2 = (data_labels[:,2] == 1.0).sum()
    #print(names[2], '_percentage: %.2f' % (q_count_train2/data_labels.shape[0]))
    ...
    for average_iter in range(average_num):
        ...
        model = ETE_ETV_Net( n, num_classes = num_classes, net_type = net_type, conv_learn = True, num_ETE = 2, num_neurons = 10, depth_of_dense=3)
        ...
        plt.title('took time [min]: ' + str(round(vtime4/60,3)) + str(names) + 'n: ' + str(n))
    ...
    file1 = open(file_dir +'/f1_precision_recall_' + str(n) + '.txt', 'w')
    L = ['model ' + str(i) + '\n' + 
    ...
    plt.title('random with types '  + str(names) + 'n: ' + str(n))
    plt.ylim(0.0, y_upper)
    ...
    plt.legend()
    plt.grid()
    np.savez(file_dir + 'average_' + str(n) + '.npz', train_loss, test_accuracy, train_accuracy, test_loss)
...
# ------------------------------------------------------------- 
for i in [1,2,3]:
    now_testing = str(i) + 'c_q/'
    comp_list = [0, 1]
    main(3, now_testing, comp_list)

    # ------------------------------------------------------------- 
    now_testing = str(i) +  'c_T/'
    comp_list = [0, 4]
    main(2, now_testing, comp_list, i)

    # ------------------------------------------------------------- 
    now_testing = str(i) +  'q_T/'
    comp_list = [1, 4]
    main(3, now_testing, comp_list, i)
